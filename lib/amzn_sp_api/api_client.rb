  ...
  class ApiClient
    ...
    def call_api(http_method, path, opts = {})
      ...
      unless response.success?
        if response.timed_out?
          ...
          raise ApiError.new(code: response.code, response_headers: response.headers, response_body: response.body),
    ...
    def convert_to_type(data, return_type)
      ...
      when /\AHash<String, (.+)>\z/
        ...
        # models, e.g. Pet
        AmznSpApi.constants
                 .map { |c| AmznSpApi.const_get(c) }
                 .select { |sub| sub.is_a?(Module) }
                 .detect { |sub| sub.const_defined?(return_type) }
