  ...
  class ItemApprovalAction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmznSpApi::OrdersApiModel::ItemApprovalAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmznSpApi::OrdersApiModel::ItemApprovalAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      action_type_validator = EnumAttributeValidator.new('Object', ['APPROVE', 'DECLINE', 'APPROVE_WITH_CHANGES'])
    ...
    def action_type=(action_type)
      validator = EnumAttributeValidator.new('Object', ['APPROVE', 'DECLINE', 'APPROVE_WITH_CHANGES'])
