  ...
  class Order
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmznSpApi::OrdersApiModel::Order` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmznSpApi::OrdersApiModel::Order`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'item_approval_types')
        if (value = attributes[:'item_approval_types']).is_a?(Array)
          self.item_approval_types = value
        end
      end

      if attributes.key?(:'item_approval_status')
        if (value = attributes[:'item_approval_status']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          item_approval_types == o.item_approval_types &&
          item_approval_status == o.item_approval_status
    end
