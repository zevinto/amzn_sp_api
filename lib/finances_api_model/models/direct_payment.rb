...
module AmznSpApi::FinancesApiModel
  ...
  class DirectPayment
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `AmznSpApi::FinancesApiModel::DirectPayment` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `AmznSpApi::FinancesApiModel::DirectPayment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)

        h[k.to_sym] = v
      end

      self.direct_payment_type = attributes[:direct_payment_type] if attributes.key?(:direct_payment_type)

      return unless attributes.key?(:direct_payment_amount)
    ...
    def ==(other)
      ...
      self.class == other.class &&
        direct_payment_type == other.direct_payment_type &&
        direct_payment_amount == other.direct_payment_amount
    end
