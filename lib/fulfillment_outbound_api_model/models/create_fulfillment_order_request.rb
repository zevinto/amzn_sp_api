...
module AmznSpApi::FulfillmentOutboundApiModel
  ...
  class CreateFulfillmentOrderRequest
    ...
    attr_accessor :displayable_order_date, :shipping_speed_category, :delivery_window, :destination_address, :fulfillment_action, :fulfillment_policy, :cod_settings, :notification_emails, :items
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `AmznSpApi::FulfillmentOutboundApiModel::CreateFulfillmentOrderRequest` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `AmznSpApi::FulfillmentOutboundApiModel::CreateFulfillmentOrderRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
      ...
      self.ship_from_country_code = attributes[:ship_from_country_code] if attributes.key?(:ship_from_country_code)

      self.notification_emails = attributes[:notification_emails] if attributes.key?(:notification_emails)

      if attributes.key?(:feature_constraints) && (value = attributes[:feature_constraints]).is_a?(Array)
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        ship_from_country_code == other.ship_from_country_code &&
