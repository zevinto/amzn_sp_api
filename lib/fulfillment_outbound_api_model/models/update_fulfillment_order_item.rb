...
module AmznSpApi::FulfillmentOutboundApiModel
  ...
  class UpdateFulfillmentOrderItem
    ...
    attr_accessor :quantity, :per_unit_declared_value, :per_unit_price, :per_unit_tax
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `AmznSpApi::FulfillmentOutboundApiModel::UpdateFulfillmentOrderItem` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `AmznSpApi::FulfillmentOutboundApiModel::UpdateFulfillmentOrderItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
      ...
      self.per_unit_declared_value = attributes[:per_unit_declared_value] if attributes.key?(:per_unit_declared_value)

      self.per_unit_price = attributes[:per_unit_price] if attributes.key?(:per_unit_price)

      return unless attributes.key?(:per_unit_tax)
